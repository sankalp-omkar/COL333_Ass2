...
double evaluate_board(
    ...
    for (int y = 0; y < rows; y++) {
        for (int x = 0; x < cols; x++) {
            ...
            if (cell.side == "stone" && cell.owner == player) {
                ...
                if (row_dist <= 5) my_stones_within_5_rows++;
                ...
                auto result = bfs_distance_to_goals(
                ...
                if (result.distance < INF) {
                    ...
                    if (result.distance < 15) {
                        ...
                        my_proximity_score += std::pow(15.0 - effective_dist, 2);
                    }
                }
            ...
            // === OPPONENT STONES ===
                ...
                if (result.distance < INF) {
                    ...
                    if (result.distance < 15) {
                        double effective_dist = std::min(result.distance, 15.0);
                        opp_proximity_score += std::pow(15.0 - effective_dist, 2);
                    }
                }
    ...
    // Row-based distances
    score += my_stones_within_5_rows * LearnedWeights::MY_STONES_WITHIN_5_ROWS * 1e5;
    ...
    // Path diversity
    score += my_clear_paths_to_goal * LearnedWeights::MY_CLEAR_PATHS_TO_GOAL * 1e5;
